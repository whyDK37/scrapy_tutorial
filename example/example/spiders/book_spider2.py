...
class BooksSpider(scrapy.Spider):
    ...
    start_urls = ['https://sobooks.cc/page/1']
    ...
    def parse(self, response):
        ...
        for book in books:
            ...
            author = book.xpath('./p/a/text()').extract_first()
            channel = book.xpath('./div/div/a/text()').extract_first()
            href = book.xpath('./div/a//@href').extract_first()
            img = book.xpath('./div/a/img//@data-original').extract_first()
            yield {
                'name': name,
                'author': author,
                'channel': channel,
